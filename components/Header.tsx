...
import { Menu, X, Globe, Phone, Download, Activity, LogIn, LogOut, Wallet, User as UserIcon, ShieldAlert, ShieldCheck, Printer, Settings } from 'lucide-react';
          ...
          <div className="hidden lg:flex space-x-6 items-center">
            ...
            {user ? (
              <div className="flex items-center gap-4">
                ...
                <button 
                  onClick={user.role === 'admin' ? onOpenAdmin : onOpenAuth}
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition shadow-lg ${user.role === 'admin' ? 'bg-blue-900 text-white shadow-blue-900/20' : 'bg-gray-100 text-gray-600'}`}
                >
                  <ShieldAlert size={14} className={user.role === 'admin' ? 'animate-pulse' : ''} /> 
                  {user.role === 'admin' ? 'Admin Panel' : 'Switch Account'}
                </button>
          ...
          <div className="px-4 pt-4 pb-6 space-y-2">
            ...
            {user ? (
              ...
              <button 
                ...
                className="w-full text-left flex items-center gap-4 text-orange-600 px-4 py-4 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-orange-50 transition-all"
              >
                <div className="p-2 bg-orange-50 rounded-xl"><LogIn size={20} /></div>
                Access Portal
              </button>
            )}
          </div>
